...
FIA <- read.csv(here("data/CA_FIA", "CA_PLOTSNAP.csv"))
...
forest.sf <- st_read(here("GIS/USFS_boundaries", "S_USA.AdministrativeForest.shp")) %>%
...
county.sf <- st_read(here("GIS/CA_counties", "CA_counties.shp")) %>%
...
DEM.1 <- raster(here("GIS/USGS_NED_13_n40w122_IMG",
                     "USGS_NED_13_n40w122_IMG.img"))
DEM.2 <- raster(here("GIS/USGS_NED_13_n41w121_IMG",
                      "USGS_NED_13_n41w121_IMG.img"))
DEM.3 <- raster(here("GIS/USGS_NED_13_n41w122_IMG",
                     "USGS_NED_13_n41w122_IMG.img"))
DEM.4 <- raster(here("GIS/USGS_NED_13_n42w122_IMG",
                     "USGS_NED_13_n42w122_IMG.img"))
DEM.5 <- raster(here("GIS/USGS_NED_13_n40w121_IMG",
                     "USGS_NED_13_n40w121_IMG.img"))
DEM.6 <- raster(here("GIS/USGS_NED_13_n40w123_IMG",
                     "USGS_NED_13_n40w123_IMG.img"))
DEM.7 <- raster(here("GIS/USGS_NED_13_n41w123_IMG",
                     "USGS_NED_13_n41w123_IMG.img"))
DEM.8 <- raster(here("GIS/USGS_NED_13_n42w121_IMG",
...
FIA_TREE <- read.csv(here("Data/CA_FIA", "CA_TREE.csv"))
FIA_COND <- read.csv(here("Data/CA_FIA", "CA_COND.csv"))
...
fia_forest_types <- read.csv(here("Data/CA_FIA", "REF_FOREST_TYPE.csv"))
fia_species_list <- read.csv(here("Data/CA_FIA", "REF_SPECIES.csv"))
...
#Removed bigleaf maple (n = 1) and mountain hemlock (n = 1).
#End with 215 plots.
...
#Removed plots where live BA < 9 m^2/ha (52 plots removed; 164 plots remain).
#End with 164 plots
...
#Summarizing tree density data to join with all tables later (see below)
tree_dens <- tree_join %>%
  group_by(PLT_CN) %>%
  summarise(DENS_12plus = mean(DENS_12plus),
            DENS_12_24 = mean(DENS_12_24),
            DENS_24_36 = mean(DENS_24_36),
            DENS_36plus = mean(DENS_36plus),
            BALIVE_sqmha = mean(BALIVE_sqmha))

#Joining species data with tree_join data.
#Start with 164 plots.
...
#Estimating hardwood and conifer BA.
#Removing plots with < 9 m^2 conifer BA (1 plot removed, 163 plots remain).
#Removing plots without Abies or Pinus spp (4 plots removed, 159 plots remain)
#Removing BALIVE_sqmha column (already in another table).
#End with 159 plots.
species_join <- inner_join(tree_join, fia_species_list, by = "SPCD") %>%
  ...
  rename(ABCO_met = ABCO,
         ...
         QUKE_met = QUKE) %>%
  group_by(PLT_CN) %>%
  mutate(hardwood.ba = ACMA_met + POBA_met + QUCH_met + QUKE_met,
         conifer.ba = BALIVE_sqmha - hardwood.ba,
         AB_PI = (ABCO_met + ABMA_met + ABSH_met)/
           (ABCO_met + ABMA_met + ABSH_met + PICO_met + 
              PIJE_met + PILA_met + PIPO_met + PIMO_met)) %>%
  filter(conifer.ba > 8.9) %>%
  filter(AB_PI >= 0) %>%
  dplyr::select(-BALIVE_sqmha)
  
#Joining tables to get all information for plots.
#Final FIA dataset has 159 plots.
...
d_FIA <- inner_join(d_FIA, tree_dens, by = "PLT_CN")
